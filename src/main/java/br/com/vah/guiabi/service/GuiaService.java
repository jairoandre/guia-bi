...
public class GuiaService extends DataAccessService<Guia> {
		...
		for (Guia guia : guias) {
			...
			if (dataReceb != null) {
				...
				report.setTempoMedioRecebimento(
						report.getTempoMedioRecebimento() + Float.valueOf(diasReceb / report.getTotalCriadas()));

			}

			if (dataAudit != null) {
				long diasAudit = (dataAudit.getTime() - data.getTime()) / 1000 / 60 / 60 / 24;
				report.setTempoMedioAuditoria(
						report.getTempoMedioAuditoria() + Float.valueOf(diasAudit / report.getTotalCriadas()));
			}

			if (dataSolic != null) {
				long diasSolic = (dataSolic.getTime() - data.getTime()) / 1000 / 60 / 60 / 24;
				report.setTempoMedioSolicitacao(
						report.getTempoMedioSolicitacao() + Float.valueOf(diasSolic / report.getTotalCriadas()));
			}

			if (dataRespo != null) {
				long diasRespo = (dataRespo.getTime() - data.getTime()) / 1000 / 60 / 60 / 24;
				report.setTempoMedioResposta(
						report.getTempoMedioResposta() + Float.valueOf(diasRespo / report.getTotalCriadas()));
			}
		}
	...
	public Guia initializeAnexos(Guia guia) {
